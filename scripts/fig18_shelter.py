#!/usr/bin/env python3
# ============================================================================
# fig18_shelter — (a) modelled temporary-shelter demand per km2 by
#                     neighbourhood; (b) heavy-damage count against
#                     heavy-damage ratio, the two prioritisation measures
# ----------------------------------------------------------------------------
# WHY THIS FIGURE
#
# (a) The İBB night-time M_w 7.5 scenario places temporary-shelter demand at
#     835,918 people. Expressed per unit area it ranges from 0 to 18,889 per
#     km2 about a median of 409, so the demand is concentrated in a small part
#     of the metropolitan area. Nothing in the manuscript currently maps a
#     consequence for people rather than for buildings.
#
# (b) The manuscript states that "the choice between rate-based and count-based
#     prioritisation materially changes which districts are flagged". Panel (b)
#     shows that directly: Spearman rank correlation between absolute heavy-
#     damage count and heavy-damage ratio is 0.840 across the 959
#     neighbourhoods, so the two orderings agree broadly but disagree at the
#     margins by hundreds of rank places. Points off the diagonal band are the
#     neighbourhoods whose priority depends on which measure is chosen.
#
# COLOUR (cpt-city). Palettes assessed with cpt_tools.assess before use:
#   ocal/indigo-orange  panel (a), REVERSED so that indigo marks high demand and
#                    orange low, as requested; the file itself runs the other
#                    way. Assessed SEQUENTIAL with monotonic lightness
#                    (L* 20-75, zero reversals) - the only palette tried here
#                    that passes cleanly, so the map can be read as a gradient
#                    rather than only against the bar. Public domain (Alan
#                    Horkan, Open Clip Art Library, 2004). Earlier candidates
#                    were rejected on measurement: hult/gr39_hult averaged 0.88
#                    brightness across its lower third and read as blank;
#                    imagej/gem-256 is near-black over its lower half and buried
#                    the low-demand periphery; imagej/cmy posterised into four
#                    colours.
#   rc/aquamarinemermaid  panel (b), clipped to 0.12-0.82 so that only the dark
#                    purple at each end is removed: the ramp runs rose through
#                    gold to blue. Two consequences to be aware of. It remains
#                    DIVERGING (L* 40-81, light centre), so the gold band falls
#                    near 493 buildings, the log-midpoint of the stock range and
#                    a value with no meaning in a dataset whose median is 1054.
#                    And because the two arms are near-equal in lightness, the
#                    low and high ends separate by hue alone and will merge in
#                    greyscale. CLIP_B=0,0.5 gives the ascending arm only, which
#                    assesses SEQUENTIAL with zero reversals and avoids both.
#                    Licence: free to use.
#   rc/autumnrose    alternative for panel (b), reversed so pale marks low stock.
#                    SEQUENTIAL, L* 35-86, zero reversals, span 51 - the third
#                    widest of the eight sequential palettes in the collection.
#                    Caveat: it shares a hue family with the orange end of the
#                    panel (a) ramp, so the two panels read as one palette even
#                    though they encode different variables. CPT_B=girlcat.cpt
#                    restores the green version, which separates cleanly.
#   rc/girlcat       alternative for panel (b). Of the 61 palettes in Restless Concepts
#                    collection only eight assess as SEQUENTIAL; girlcat has the
#                    second-widest lightness span among them (L* 32-92, zero
#                    reversals) and the widest that is not near-black at one
#                    end. It runs pale yellow-green to dark green, so building
#                    stock reads as one ordered dimension. rc/daisyfae, tried
#                    first, is DIVERGING: its pale centre fell at 493 buildings,
#                    a value with no meaning in a dataset whose median is 1054,
#                    and read as a threshold that does not exist. Licence: free
#                    to use. Alternatives supplied: scarletdragon.cpt (L* 7-73,
#                    also sequential, warmer), cold.cpt, daisyfae.cpt.
#   esri/current_dir_1  requested but NOT used: it is cyclic (16 reversals,
#                    L* span only 29) and, more decisively, the ESRI collection
#                    carries a disclaimer with patent prohibition and US export
#                    restrictions, which is not a licence to embed in a journal
#                    figure.
#
# INPUT: mahalle_inventory_scenario_joined.csv, mahalle_vs30.csv (for area),
#        mahalle_with_coords.csv, mahalle_fixed.geojson, ne_10m_land.*,
#        and the two .cpt files copied beside this script.
# Output: fig18_shelter.pdf / .png
# ============================================================================
import os, sys, json
import numpy as np
import pandas as pd
