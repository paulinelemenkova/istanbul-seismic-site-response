#!/usr/bin/env python3
# =====================================================================
# fig_marmara_soils  --  3rd-level FAO-74 soil mapping units of the
# Marmara region, draped over a grey SRTM hillshade, with italic
# unit-code labels on every polygon and thin brown unit outlines.
# Engine: GMT 6.5 / PyGMT.
# =====================================================================
...
SHP = "data/DSMW/DSMW.shp"          # FAO/UNESCO DSMW shapefile (place under data/)
RELIEF = "data/relief_marmara.nc"   # SRTM15+ relief cut to the window (place under data/)
...
# 17 distinct pastel colours (Pastel1 + Pastel2 + Set3, blues removed)
...
# ---- load + clip DSMW; classify at soil-mapping-unit (3rd) level ----
...
def smu_code(f):
    ...
    return m.group(1) if m else f.split("-")[0]          # 'I-Bd-2c' -> 'I'
...
# ---- relief lattice + rasterise soils ----
...
# polygonise displayed raster -> boundaries + label points
...
lab = bnd[bnd["area"] > 1.2e8].copy()                    # label polygons > ~120 km^2
def _n_labels(a):
...
# Aegean label point (most-open SW water, off the frame)
water = land != 1
...
fig.plot(data=bnd, pen="0.25p,139/69/19")               # brown unit outlines

# italic unit-code labels; large polygons get up to 3 spread labels
_placed = []
...
# cities (capital Istanbul emphasised: red star, label in clear grid cell)
...
# single title = legend header; soil mapping units (3rd level)
...
fig.savefig("fig_marmara_soils.pdf")
fig.savefig("fig_marmara_soils.png", dpi=300)
