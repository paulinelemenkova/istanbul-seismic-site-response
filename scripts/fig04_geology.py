#!/usr/bin/env python3
# =====================================================================
# marmara_geology.py  -  Generalized geology of the Marmara region
# geopandas + matplotlib.  USGS OFR 97-470I (1:5,000,000).
# Layers: bedrock geology coloured by age (GLG), geologic provinces as
# transparent hatched overlays, faults, city/sea/province labels.
# Rebuilt from data + palette sampled from the reference figure.
# =====================================================================
...
def P(lon,lat):                       # lon/lat -> Web-Mercator metres
...
ax=fig.add_axes([0.060,0.112,0.892,0.700])     # main map
...
# --- sea / water-body labels (italic blue)  [* = positions per request]
...
sealab('Sea of Marmara',28.1833,40.75,24)                 # * 28°11'E 40°45'N
sealab('Black Sea',29.25,41.3333,30)                      # * 29°15'E 41°20'N
sealab('Aegean Sea',26.12,40.02,22,rot=90)                # * rotated 90° CCW, left border
sealab('Black Sea Deep-Water Basin',29.55,41.70,16,col='#2266aa',rot=0)

# --- province italic labels on map
...
# --- scale bar INSIDE map (bottom-right corner), ~50 km, compact
sb_lat=39.63; sb_lon0=30.30
dlon=50.0/(111.32*np.cos(np.radians(sb_lat)))           # 50 km in degrees lon
...
# alternating black/white checker along edges
...
checker_h(Y0,False)            # bottom
checker_h(Y1-th_h,False)       # top
checker_v(X0)                  # left
checker_v(X1-tw_v)             # right
# tick labels: bottom (lon) + left (lat) only  [EDIT 4: top coords removed]
...
# =====================================================================
# TOP LEGEND PANEL  [EDIT 1: grey background removed -> transparent]
# =====================================================================
...
# province strip (fixed equal slots so all six fit)
...
# =====================================================================
# BOTTOM: geologic-age ramp (left) + credit (right, same level)
# =====================================================================
...
# credit  [EDIT 3: on the same level as the bottom legend (age ramp)]
...
fig.savefig('marmara_geology.png',dpi=200,facecolor='white')
print('saved marmara_geology.png')
